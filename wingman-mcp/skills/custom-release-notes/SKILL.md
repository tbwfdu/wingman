...
1. Fetch `https://docs.omnissa.com/sitemap.xml`. This is a sitemap *index* — it doesn't list pages directly, it lists further `sitemap.xml` files (e.g. `https://docs.omnissa.com/sitemappart/1.xml`, `.../2.xml`, and so on).
   - **If this fetch errors outright** (not just large — actually fails), don't immediately treat the whole approach as dead and jump to a web search. Retry once — some fetch tools choke on XML content-type or have transient hiccups on this specific host. If it still fails, the sub-sitemaps follow a stable, predictable numbered pattern (`sitemappart/1.xml`, `sitemappart/2.xml`, `sitemappart/3.xml`, …) — try fetching those directly instead of the index. This is a real, verified pattern, not a guess: the index resolves to exactly these paths as of this writing. Walk a handful of them (1 through, say, 8-10) even without the index confirming the exact count; a release-notes bundle for a major product is very likely to turn up in the first few.
