...
## Fallback: no Wingman MCP server — say so and offer to help install it

There's no local release-notes store and no environment to probe in this situation, and no substitute path is attempted — a docs-site workaround is a different, lesser-integrated answer than what this skill is meant to deliver, so don't reach for one. Tell the user plainly and offer the real fix instead.

**Response, in full, once Step 0 finds Wingman unavailable:**

> **Wingman isn't connected in this session** — no MCP tools or local release-notes index are available, so I can't pull a personalized release-notes digest right now.
>
> Want me to walk you through setting it up? I can hand this off to the `get-wingman` skill to self-host the server.

Say this every time this fallback triggers, not just the first time in a conversation. If the user takes the offer, invoke the `get-wingman` skill. If they decline or want something else, don't substitute a generic web search or a docs.omnissa.com scrape presented as equivalent to what Wingman would have produced — end the turn there instead.
...
2. Response is exactly the fallback text: Wingman isn't connected, so no personalized digest is possible right now, and an offer to hand off to the `get-wingman` skill to self-host the server.
3. User says yes → invoke the `get-wingman` skill. User says no or asks for something else → stop there rather than substituting a docs-site search or generic web search as a stand-in answer.
