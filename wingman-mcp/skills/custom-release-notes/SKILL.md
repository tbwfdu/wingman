...
**Finding the right release-notes page for a named product.** Don't guess a bundle id. The site's search endpoint can be scoped to release-notes content specifically and sorted by how recently each page was revised:

```
GET https://docs-be.omnissa.com/api/search?groupByPub=false&labelkey=release_notes&sort.field=lastRevised&sort.value=desc&page=<N>&rpp=<small>
```

That label covers every product's release notes site-wide as one manageable list, so a short paginated scan through it — matching page/bundle titles against the product the user named — reliably lands on the right one without knowing its id up front. Keep each request's page size modest and page through rather than requesting everything in one call; an oversized single request risks getting cut off by a fetch tool's own output limit before the result list finishes arriving, which would silently drop candidates from the scan.

Once the right bundle is matched, fetch its single-page detail (the `/api/bundle/<bundle_id>/page/<page_id>.html` form on the `-be` host) to get the full article body. Some products stack many historical versions on one page — use only the topmost (most recent) version's content; older versions further down aren't needed here and are safe to ignore if a long page comes back only partially.
...
4. Search `https://docs-be.omnissa.com/api/search?groupByPub=false&labelkey=release_notes&sort.field=lastRevised&sort.value=desc&page=1&rpp=5` (and further pages as needed) for a title matching Workspace ONE UEM; match found, its bundle_id and page_id noted.
