...
**Finding the right release-notes page for a named product.** Don't guess a bundle id. Do this discovery on the same `docs-be.omnissa.com` backend host used for page fetches above — not the plain `docs.omnissa.com` host. The plain host isn't just JS-rendered for individual bundle pages; in practice it has also proven unreliable to fetch *at all* for static assets like its sitemap (a direct fetch of `docs.omnissa.com/sitemap.xml` has failed outright from inside a chat client, even though the equivalent `docs-be.omnissa.com/api/...` JSON calls succeeded). Route discovery through `-be` too, not just page content.

The backend search endpoint, scoped to release-notes content and sorted by recency:

```
GET https://docs-be.omnissa.com/api/search?groupByPub=false&labelkey=release_notes&sort.field=lastRevised&sort.value=desc&rpp=1&page=<N>
```

Use `rpp=1` and increment `page` rather than asking for a larger page size in one call — this isn't a style preference. A larger `rpp` on this endpoint has been observed to get cut off by a fetch tool's own output-size limit before the result list finishes arriving, silently dropping candidates from the scan, while `rpp=1` reliably returns one complete, well-formed result every call. Confirm a call actually completed by checking that `"Pagination":{` appears in what came back; if it's missing, that call was cut short — retry it rather than counting it as "no match here."

There are only on the order of 70-something release-notes bundles site-wide, so a full scan is bounded (well under 100 calls at worst), and you can stop as soon as a match for the named product turns up rather than paging through all of them. Match against `leading_result.bundle_id` / `leading_result.title` for the product's name — release-notes bundle names typically encode product and version together (e.g. `Workspace-ONE-UEM-Release-NotesV2602`). One quirk to know: passing `labelkey` more than once to narrow further (e.g. adding a second, product-specific label) doesn't work — the endpoint only honors the first value and silently ignores the rest, so scanning the single `release_notes`-filtered list and matching client-side, as above, is the reliable path, not a second label.

It's normal to end up with more than one matching bundle per product — release-notes bundles are often versioned (e.g. `...V2506`, `...V2602`), with each version getting its own bundle and its own page. Prefer the highest/most recent version number found; that's the current one.
...
**If the search endpoint itself is unreachable** (not just slow — genuinely erroring), retry once before concluding it's down. If it still fails, a static sitemap crawl of the plain `docs.omnissa.com` host is a documented alternative — it's what Wingman's own internal documentation-ingestion pipeline uses server-side (fetch `docs.omnissa.com/sitemap.xml`, walk the nested `sitemappart/1.xml`, `sitemappart/2.xml`, … files it points to, filter `<loc>` entries by bundle name). Treat it as a fallback worth trying, though, not the default — that pipeline runs as a full HTTP client with no such restriction, while a chat client's fetch tool has, in practice, handled the JSON backend host more reliably than raw XML on the public host.
...
**If both the search endpoint and the sitemap fallback are genuinely exhausted** — don't quietly pivot to a generic web search and present whatever comes back as the answer. A plain search for something like "docs.omnissa.com sitemappart 1.xml" or "<product> release notes" does not reliably return the actual release-notes bundle — in practice it surfaces unrelated pages (other products, unrelated sites, even Wikipedia) that only superficially match on keywords, and treating that as a source risks summarizing the wrong product's content with false confidence. If a web search result's URL is clearly a real `docs.omnissa.com/bundle/.../page/....html` page for the right product, it's fine to fetch and use that specific page — but a search snippet or an unrelated-domain result is not a substitute for the actual release notes, and shouldn't be presented as one.

If discovery genuinely fails end to end, say so plainly: tell the user the automated lookup didn't find the release-notes page in this environment, and ask them to paste a `docs.omnissa.com` URL if they already have one (as happened here) rather than guessing from search snippets.
...
4. Call `https://docs-be.omnissa.com/api/search?groupByPub=false&labelkey=release_notes&sort.field=lastRevised&sort.value=desc&rpp=1&page=1` and increment `page`, checking `"Pagination":{` closes on each call; a match like `Workspace-ONE-UEM-Release-NotesV2602` turns up within the first several pages (highest version number among any matches, so the current one).
