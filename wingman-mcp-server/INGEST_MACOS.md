...
> **Maintainer-only.** The `ingest` and `check` commands are not part of
> the public CLI. They require `WINGMAN_MCP_ADMIN=1` to run, and the
> `wingman_mcp_server.ingest` subpackage is excluded from the published wheel.
> Operate from a source checkout. Examples below assume that env var is
...
To rebuild just one product's release notes:
    ...
    wingman-mcp-server ingest horizon_rn

To rebuild all products' release notes:

    wingman-mcp-server ingest rn
...
wingman-mcp-server check

# Or one at a time
wingman-mcp-server check api
wingman-mcp-server check uem
wingman-mcp-server check release_notes
```
...
4. Save it as plain text named `v{version}_rn.txt` **in the repo root** (the same directory you'll run `wingman-mcp-server ingest` from). So you should end up with:
...
```bash
wingman-mcp-server ingest
```

### Or refresh one store at a time

```bash
wingman-mcp-server ingest api
wingman-mcp-server ingest release_notes
wingman-mcp-server ingest uem
```
...
Example:
```bash
wingman-mcp-server ingest uem --max-workers 20 --batch-size 200
```
...
- **`ModuleNotFoundError: No module named 'requests'`**
  You didn't install the `[ingest]` extras. Rerun `pip install -e .`.
